...
const MicroDigest = (() => {
  ...
  function mountUI(panel) {
    filePicker = RetroFileInput.create({
      id: 'mdigest-file',
      label: 'UPLOAD DOCUMENT (.txt / .md)',
      accept: '.txt,.md,text/plain,text/markdown',
      ariaLabel: 'Upload document (.txt or .md)',
      variant: 'orange',
      placeholder: 'NO FILE SELECTED',
      extensions: ['txt', 'md'],
      invalidMessage: 'USE .txt OR .md ONLY',
    ...
    root.innerHTML = `
      <div class="px-box mt-16" style="border-color:var(--orange);">
        ...
        <div id="mdigest-file-meta" style="font-family:var(--font-vt);font-size:16px;color:var(--gray-light);margin-top:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">NO FILE LOADED</div>
      </div>
  ...
  function onFileSelected(file) {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        if (meta) {
          const line = file.name + ' — ' + sourceText.length + ' chars, ' + wordCount(sourceText) + ' words';
      ...
      reader.onerror = () => {
        Toast.show('FILE READ ERROR', 'error');
