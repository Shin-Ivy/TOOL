...
const StemSep = (() => {
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:16px;">
        <div style="flex:1;min-width:260px;">
          <div class="px-box" style="border-color:var(--blue);">
            <label class="px-label" for="stem-file">UPLOAD AUDIO (.mp3 / .wav)</label>
            <input type="file" id="stem-file" accept=".mp3,.wav,audio/mpeg,audio/wav,audio/wave"
              style="font-family:var(--font-vt);font-size:18px;color:var(--green);margin-top:8px;width:100%;">
            <div id="stem-meta" style="font-family:var(--font-vt);font-size:16px;color:var(--gray-light);margin-top:8px;">NO TRACK LOADED</div>
          </div>
  ...
  function bindEvents() {
    document.getElementById('stem-file')?.addEventListener('change', async (e) => {
      const f = e.target.files?.[0];
      if (!f) return;
      const ext = (f.name.split('.').pop() || '').toLowerCase();
      if (!['mp3', 'wav'].includes(ext)) {
        Toast.show('USE .mp3 OR .wav', 'error');
        return;
      }
      try {
        await decodeFile(f);
